...
void setup() {
    ...
    Serial.println();
    Serial.println("===========================");
    Serial.println("  ESP32 BLE Spam - v1.4");
    Serial.println("===========================");
    Serial.println();
...
void handleRoot() {
    ...
    String html = "<html><head><title>ESP32 BLE Spam</title></head><body>";
    ...
    html += "<script>";
    html += "function startSpam(){fetch('/start').then(()=>{document.getElementById('status').textContent='SPAMMING'})";
    html += "function stopSpam(){fetch('/stop').then(()=>{document.getElementById('status').textContent='STOPPED'})";
        ...
        xTaskCreatePinnedToCore(
            [](void* param) {
                ...
                while (spamming) {
                    ...
                    else if (current_device == 2) adv.setName("Samsung Buds");
